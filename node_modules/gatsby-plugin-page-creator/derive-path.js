...
async function derivePath(path, node, reporter, slugifyOptions, getFieldValue) {
  ...
  for (const slugPart of slugParts) {
    ...
    const key = (0, _pathUtils.switchToPeriodDelimiters)(cleanedField); // 3.b We do node.nodes here because we support the special group graphql field, which then moves nodes in another depth

    const groupNodes = (0, _get2.default)(node.nodes, `[0]${key}`); // In case not all nodes are materialized yet (e.g. in setFieldsOnGraphQLNodeType or createResolvers) it's not enough to just _.get the node value, but to call the helper function nodeModel.getFieldValue. nodeModel is not always accessible (not passed, not available, etc.) so the argument is optional and falls back to _.get.

    const singleNode = getFieldValue ? await getFieldValue(node, key) : (0, _get2.default)(node, key);
    const nodeValue = groupNodes || singleNode; // 3.c log error if the key does not exist on node
